...
void CClientScoreBoardDialog::FillScoreBoard()
{
	...
	for( int i = FF_TEAM_BLUE; i <= FF_TEAM_GREEN; i++ )
	{
		...
		// Get team score
		m_hSections[ i ].m_iScore = g_FF_PR->GetTeamScore( iTeam );
		// Get the time the team last scored
		m_hSections[ i ].m_flLastScored = g_FF_PR->GetTeamScoreTime( iTeam );

		int iPlayerCount = 0, iLatency = 0;

		for( int j = 1; j <= gpGlobals->maxClients; j++ )
		{
			if( g_FF_PR->IsConnected( j ) )
			{
				// If the player is on iTeam
				if( g_FF_PR->GetTeam( j ) == iTeam )
				{
					iPlayerCount++;
					iLatency += g_FF_PR->GetPing( j );
				}
			}
		}
