...
void CBaseEffect::Render(int x, int y, int w, int h)
{
	...
	pMatRenderContext->DrawScreenSpaceRectangle(m_Material, x, y, w, h,
	...
	if (flInverseAlpha > 0.0f)
	{
		...
		pMatRenderContext->DrawScreenSpaceRectangle(pMatScreen, x, y, w, h,
			actualRect.x, actualRect.y, actualRect.x+actualRect.width-1, actualRect.y+actualRect.height-1, 
			pTexture->GetActualWidth(), pTexture->GetActualHeight());
	}
}
...
void CMotionBlur::Init()
{
	...
	m_BlurImage.InitRenderTarget(256, 256, RT_SIZE_FULL_FRAME_BUFFER,
		IMAGE_FORMAT_ARGB8888, MATERIAL_RT_DEPTH_NONE, false, "CMotionBlur::Init");
}
...
void CMotionBlur::Render(int x, int y, int w, int h)
{
	...
	ITexture *pOriginalRenderTarget = pMatRenderContext->GetRenderTarget();
	...
	if(gpGlobals->curtime >= m_flNextSampleTime) 
	{
		...
		pMatRenderContext->SetRenderTarget(m_BlurImage);
		pMatRenderContext->DrawScreenSpaceQuad(pMatScreen);
	...
	// We're now drawing
	pMatRenderContext->SetRenderTarget(pOriginalRenderTarget);
	pMatRenderContext->DrawScreenSpaceQuad(pMatScreen);
