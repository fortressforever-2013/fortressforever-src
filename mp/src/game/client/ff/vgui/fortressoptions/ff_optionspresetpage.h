...
namespace vgui
{
	...
	public:
		...
		// Purpose: Catch name input dialog's okay button and process the text
		//-----------------------------------------------------------------------------
		MESSAGE_FUNC_PARAMS(OnShowNameInputCommand, "ShowNameInput", data);
	...
	protected:
		...
		void ShowNameInput(const char* szDefaultName = NULL);
		void ShowNameInputError(const char* szReason, const char* szDefaultName = NULL);
	};
};
