...
void CFFMiscOptions::Apply()
{
	...
	for (KeyValues *kvOption = kvOptions->GetFirstSubKey(); kvOption != NULL; kvOption = kvOption->GetNextKey())
	{
		...
		else if (ComboBox *cb = dynamic_cast <ComboBox *> (pChild))
		{
			...
			g_pVGuiLocalize->ConvertUnicodeToANSI(g_pVGuiLocalize->Find("#GameUI_Custom"), szCustomCaption, sizeof(szCustomCaption));
...
void CFFMiscOptions::Load()
{
	...
	for (KeyValues *kvOption = kvOptions->GetFirstSubKey(); kvOption != NULL; kvOption = kvOption->GetNextKey())
	{
		...
		wchar_t* wszCaption = g_pVGuiLocalize->Find(pszCaption);
		char szCaption[128];
		if(wszCaption)
			g_pVGuiLocalize->ConvertUnicodeToANSI(wszCaption, szCaption, sizeof(szCaption));
		else
		...
		else if (Q_strncmp(pszType, "discrete", 8) == 0)
		{
			...
			while (kvValue)
			{
				...
				wchar_t* wszItemCaption = g_pVGuiLocalize->Find(pszItemCaption);
				char szItemCaption[128];
				if(wszItemCaption)
					g_pVGuiLocalize->ConvertUnicodeToANSI(wszItemCaption, szItemCaption, sizeof(szItemCaption));
				else
