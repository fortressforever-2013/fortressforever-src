...
void CFFCustomHudAssignPresets::Load()
{
	...
	if (!m_bLoaded)
	{
		...
		m_kvLoadedAssignments->LoadFromFile(g_pFullFileSystem, PRESETASSIGNMENT_FILE);
...
void CFFCustomHudAssignPresets::Apply() {
	...
	kvPresets->SaveToFile(g_pFullFileSystem, PRESETASSIGNMENT_FILE);
...
KeyValues* CFFCustomHudAssignPresets::GetStyleDataForAssignment(KeyValues* kvAssignment)
{
	...
	wchar_t* wszDefault = g_pVGuiLocalize->Find("#GameUI_Default");
	char szDefault[32];
	g_pVGuiLocalize->ConvertUnicodeToANSI(wszDefault, szDefault, sizeof(szDefault));
...
void CFFCustomHudAssignPresets::ApplyAssignmentToControls(KeyValues* kvAssignment)
{
	...
	if (kvPanelSpecificOptions)
	{
		// We could get the text sizes for each column and size accordingly
		// but for now we'll just divide the panel width by the number of groups
		int iMaxGroup = 0;
		for (KeyValues* kvPanelSpecificOption = kvPanelSpecificOptions->GetFirstSubKey();
			kvPanelSpecificOption != NULL;
			kvPanelSpecificOption = kvPanelSpecificOption->GetNextKey())
		{
			int iGroup = kvPanelSpecificOption->GetInt("group", 0);
		...
		int iGroups = iMaxGroup + 1;
		int iGroupWidth = m_pPanelSpecificOptions->GetWide() / iGroups;
		int iLabelWidth = iGroupWidth / 2 - 5;
		int iRowHeight = 30;
		int* iyCoords = new int[iGroups];
		...
		for (KeyValues* kvPanelSpecificOption = kvPanelSpecificOptions->GetFirstSubKey();
			...
			kvPanelSpecificOption = kvPanelSpecificOption->GetNextKey())
		{
			const int iGroup = kvPanelSpecificOption->GetInt("group");
			const char* pszName = kvPanelSpecificOption->GetString("name");
			const char* pszText = kvPanelSpecificOption->GetString("text");
			const wchar_t* wszText = g_pVGuiLocalize->Find(pszText);
			int iYCoord = iyCoords[iGroup]++ * iRowHeight;
			...
			if (Q_strnicmp(kvPanelSpecificOption->GetName(), "boolean", 7) == 0)
			{
				CheckButton* cb
					= !!wszText
					? new CheckButton(m_pPanelSpecificOptions, pszName, wszText)
					: new CheckButton(m_pPanelSpecificOptions, pszName, pszText);
				...
				cb->SetPos(iGroupWidth * iGroup, iYCoord);
				cb->SetSize(iGroupWidth - 10, iRowHeight);

				if (kvPanelSpecificValues)
					cb->SetSelected(kvPanelSpecificValues->GetInt(pszName, kvPanelSpecificOption->GetInt("defaultValue")));
				else
			...
			else if (Q_strnicmp(kvPanelSpecificOption->GetName(), "combobox", 8) == 0)
			{
				...
				ComboBox* cb = new ComboBox(
					m_pPanelSpecificOptions,
					kvPanelSpecificOption->GetString("name"),
					nValues, false);
				...
				while (kvValue)
				{
					...
					const wchar_t* wszCaption = g_pVGuiLocalize->Find(pszCaption);
				...
				cb->SetPos(iGroupWidth * iGroup, iYCoord);
				cb->SetSize(iLabelWidth, iRowHeight);
				...
				Label* l
					= !!wszText
					? new Label(m_pPanelSpecificOptions, pszName, wszText)
					: new Label(m_pPanelSpecificOptions, pszName, pszText);

				l->SetPos(iGroupWidth * iGroup + iLabelWidth + 10, iYCoord);
				l->SetSize(iLabelWidth, iRowHeight);
			}
		}
...
void CFFCustomHudAssignPresets::RegisterAssignmentChange(KeyValues* kvSelf, KeyValues* kvParent)
{
	...
	bool bPreviewMode = (kvSelf->GetInt("previewMode", -1) == 1);

	if (bPreviewMode)
	{
		SendPresetPreviewDataToAssignment(kvSelf, kvSelf);
	}
...
void CFFCustomHudAssignPresets::OnAddQuantityPanel(KeyValues* data)
{
	...
	if (!(Q_stricmp(data->GetString("parentName"), "") == 0))
		//if it has a parent
		//like SG, Dispenser, ManCannon might all be considered a buildable
...
void CFFCustomHudAssignPresets::OnUpdateCombos(KeyValues* data)
{
	...
	{
		RegisterAssignmentChange("Position", m_pPositionPresets);
	}
	else if (data->GetPtr("panel") == m_pPanelStylePresets)
	{
		RegisterAssignmentChange("PanelStyle", m_pPanelStylePresets);
	}
	else if (data->GetPtr("panel") == m_pItemStylePresets)
	{
		RegisterAssignmentChange("ItemStyle", m_pItemStylePresets);
	}
	//panel specific options
	else
	{
...
void CFFCustomHudAssignPresets::RegisterAssignmentChange(
	const char* keyName,
	ComboBox* comboBox)
{
	if (m_pSecondary->IsVisible())
		//if the assignment is in secondary
	{
		...
		KeyValues* kvSecondary = m_pSecondary->GetActiveItemUserData();
		kvSecondary->SetString(keyName, comboBox->GetActiveItemUserData()->GetName());

		KeyValues* kvPrimary = m_pPrimary->GetActiveItemUserData();
		KeyValues* kvAssignment = kvPrimary->FindKey(kvSecondary->GetName());

		kvAssignment->SetString(keyName, comboBox->GetActiveItemUserData()->GetName());
	...
	{
		KeyValues* kvPrimary = m_pPrimary->GetActiveItemUserData();
		kvPrimary->SetString(keyName, comboBox->GetActiveItemUserData()->GetName());
...
void CFFCustomHudAssignPresets::PositionPresetDeleted(const char* pszDeletedPresetName)
{
	...
	for (int rowIndex = 0; rowIndex < m_pPositionPresets->GetItemCount(); ++rowIndex)
	{
		...
		if (Q_stricmp(itemText, pszDeletedPresetName) == 0)
		{
			...
			// TODO this didn't seem to work as it appeared to be still
			// selected, even though it didn't exist.
			// May be a problem with the dropdown not updating the visible text.
