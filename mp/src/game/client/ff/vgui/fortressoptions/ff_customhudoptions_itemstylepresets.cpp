...
namespace vgui
{
	CFFCustomHudItemStylePresets::CFFCustomHudItemStylePresets(Panel* parent, char const* panelName, char const* pszComboBoxName) : BaseClass(parent, panelName, pszComboBoxName, ITEMSTYLEPRESET_FILE)
	{
		...
		m_pItemMarginHorizontal->SetRange(0, 20, 0, 60);
		...
		m_pItemMarginVertical->SetRange(0, 20, 0, 60);
		...
		kv = new KeyValues("AH");
		kv->SetInt("Left", FFQuantityHelper::ALIGN_LEFT);
		m_pAlignH->AddItem("#GameUI_Left", kv);
		kv->deleteThis();
		kv = new KeyValues("AH");
		kv->SetInt("Center", FFQuantityHelper::ALIGN_CENTER);
		m_pAlignH->AddItem("#GameUI_Center", kv);
		kv->deleteThis();
		kv = new KeyValues("AH");
		kv->SetInt("Right", FFQuantityHelper::ALIGN_RIGHT);
		...
		kv = new KeyValues("AV");
		kv->SetInt("Top", FFQuantityHelper::ALIGN_TOP);
		m_pAlignV->AddItem("#GameUI_Top", kv);
		kv->deleteThis();
		kv = new KeyValues("AV");
		kv->SetInt("Middle", FFQuantityHelper::ALIGN_MIDDLE);
		m_pAlignV->AddItem("#GameUI_Middle", kv);
		kv->deleteThis();
		kv = new KeyValues("AV");
		kv->SetInt("Bottom", FFQuantityHelper::ALIGN_BOTTOM);
	...
	KeyValues* CFFCustomHudItemStylePresets::RemoveNonEssentialValues(KeyValues* kvPreset)
	{
			...
			//if it does
		{
			...
			kvTempComponent->SetInt("anchorPosition", kvComponent->GetInt("anchorPosition", FFQuantityHelper::ANCHORPOS_MIDDLELEFT));
			kvTempComponent->SetInt("alignH", kvComponent->GetInt("alignH", FFQuantityHelper::ALIGN_RIGHT));
			kvTempComponent->SetInt("alignV", kvComponent->GetInt("alignV", FFQuantityHelper::ALIGN_MIDDLE));
			...
			//if it does
		{
			...
			kvTempComponent->SetInt("anchorPosition", kvComponent->GetInt("anchorPosition", FFQuantityHelper::ANCHORPOS_TOPLEFT));
			kvTempComponent->SetInt("alignH", kvComponent->GetInt("alignH", FFQuantityHelper::ALIGN_LEFT));
			kvTempComponent->SetInt("alignV", kvComponent->GetInt("alignV", FFQuantityHelper::ALIGN_BOTTOM));
			...
			//if it does
		{
			...
			kvTempComponent->SetInt("shadow", kvComponent->GetInt("shadow", 1));
			kvTempComponent->SetInt("size", kvComponent->GetInt("size", 1));
			kvTempComponent->SetInt("anchorPosition", kvComponent->GetInt("anchorPosition", FFQuantityHelper::ANCHORPOS_TOPRIGHT));
			kvTempComponent->SetInt("alignH", kvComponent->GetInt("alignH", FFQuantityHelper::ALIGN_RIGHT));
			kvTempComponent->SetInt("alignV", kvComponent->GetInt("alignV", FFQuantityHelper::ALIGN_BOTTOM));
			kvTempComponent->SetInt("offsetX", kvComponent->GetInt("offsetX", 0));
			kvTempComponent->SetInt("offsetY", kvComponent->GetInt("offsetY", 0));
	...
	void CFFCustomHudItemStylePresets::UpdatePresetFromControls(KeyValues* kvPreset)
	{
		...
		if (m_pSticky->IsEnabled())
			kvComponent->SetInt("sticky", m_pSticky->IsSelected());
		...
		if (m_pAnchorPositionTopLeft->IsEnabled() && m_pAnchorPositionTopLeft->IsSelected())
			...
			kvComponent->SetInt("anchorPosition", FFQuantityHelper::ANCHORPOS_TOPCENTER);
			...
			kvComponent->SetInt("anchorPosition", FFQuantityHelper::ANCHORPOS_MIDDLECENTER);
		else if (m_pAnchorPositionMiddleRight->IsEnabled() && m_pAnchorPositionMiddleRight->IsSelected())
			kvComponent->SetInt("anchorPosition", FFQuantityHelper::ANCHORPOS_MIDDLERIGHT);
		else if (m_pAnchorPositionBottomLeft->IsEnabled() && m_pAnchorPositionBottomLeft->IsSelected())
			kvComponent->SetInt("anchorPosition", FFQuantityHelper::ANCHORPOS_BOTTOMLEFT);
		else if (m_pAnchorPositionBottomCenter->IsEnabled() && m_pAnchorPositionBottomCenter->IsSelected())
			kvComponent->SetInt("anchorPosition", FFQuantityHelper::ANCHORPOS_BOTTOMCENTER);
		else if (m_pAnchorPositionBottomRight->IsEnabled() && m_pAnchorPositionBottomRight->IsSelected())
			kvComponent->SetInt("anchorPosition", FFQuantityHelper::ANCHORPOS_BOTTOMRIGHT);
	...
	void CFFCustomHudItemStylePresets::UpdateComponentControls(KeyValues* kvComponent)
	{
		...
		m_pSticky->RemoveActionSignalTarget(this);
		...
		m_pSticky->SetSelected(kvComponent->GetInt("sticky", 0));
		...
		if (!(Q_stricmp(m_szName, "Bar") == 0 || Q_stricmp(m_szName, "BarBorder") == 0 || Q_stricmp(m_szName, "BarBackground") == 0))
		{
			if (Q_stricmp(m_szName, "Icon") == 0)
			{
				...
				m_pSize->SetRange(1, QUANTITYITEMICONSIZES);
			}
			else
			{
			...
			m_pAnchorPositionTopLeft->SetSelected(kvComponent->GetInt("anchorPosition", -1) == FFQuantityHelper::ANCHORPOS_TOPLEFT);
			m_pAnchorPositionTopCenter->SetSelected(kvComponent->GetInt("anchorPosition", -1) == FFQuantityHelper::ANCHORPOS_TOPCENTER);
			m_pAnchorPositionTopRight->SetSelected(kvComponent->GetInt("anchorPosition", -1) == FFQuantityHelper::ANCHORPOS_TOPRIGHT);
			m_pAnchorPositionMiddleLeft->SetSelected(kvComponent->GetInt("anchorPosition", -1) == FFQuantityHelper::ANCHORPOS_MIDDLELEFT);
			m_pAnchorPositionMiddleCenter->SetSelected(kvComponent->GetInt("anchorPosition", -1) == FFQuantityHelper::ANCHORPOS_MIDDLECENTER);
			m_pAnchorPositionMiddleRight->SetSelected(kvComponent->GetInt("anchorPosition", -1) == FFQuantityHelper::ANCHORPOS_MIDDLERIGHT);
			m_pAnchorPositionBottomLeft->SetSelected(kvComponent->GetInt("anchorPosition", -1) == FFQuantityHelper::ANCHORPOS_BOTTOMLEFT);
			m_pAnchorPositionBottomCenter->SetSelected(kvComponent->GetInt("anchorPosition", -1) == FFQuantityHelper::ANCHORPOS_BOTTOMCENTER);
			m_pAnchorPositionBottomRight->SetSelected(kvComponent->GetInt("anchorPosition", -1) == FFQuantityHelper::ANCHORPOS_BOTTOMRIGHT);
		}
		else
		{
			m_pFontTahoma->SetEnabled(false);
		...
		if (Q_stricmp(m_szName, "Amount") == 0)
		{
			m_pSticky->SetVisible(true);
			m_pSticky->SetEnabled(true);
		}
		else
		{
			m_pSticky->SetVisible(false);
			m_pSticky->SetEnabled(false);
			m_pShadow->SetSelected(false);
		}

		m_pShow->AddActionSignalTarget(this);
		m_pSticky->AddActionSignalTarget(this);
