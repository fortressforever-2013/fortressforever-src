...
//-----------------------------------------------------------------------------
class CHudPlayerTotalScore : public CHudElement, public vgui::Panel
{
public:
	DECLARE_CLASS_SIMPLE( CHudPlayerTotalScore, vgui::Panel );

	CHudPlayerTotalScore( const char *pElementName ) : CHudElement( pElementName ), vgui::Panel( NULL, "HudPlayerTotalScore" )
	{
...
void CHudPlayerTotalScore::MsgFunc_SetPlayerTotalFortPoints( bf_read &msg )
{
	...
	char szString3[ 1024 ];
	Q_snprintf( szString3, sizeof(szString3), "%li", msg.ReadLong() );

	char szString4[ 1024 ];
	Q_snprintf( szString4, sizeof(szString4), "Fortress Points" );

	// convert int-string to unicode
	g_pVGuiLocalize->ConvertANSIToUnicode( szString3, m_pTextTotalScore, sizeof( m_pTextTotalScore ) );
	g_pVGuiLocalize->ConvertANSIToUnicode( szString4, m_pTextTotalDesc, sizeof( m_pTextTotalDesc ) );
