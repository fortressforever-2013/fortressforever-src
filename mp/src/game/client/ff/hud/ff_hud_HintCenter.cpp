...
void CHudHintCenter::AddHudHint( unsigned short hintID, short NumShow, short hintPriority, const char *pszMessage )
{
	...
	if( !pszTemp )
	{
		g_pVGuiLocalize->ConvertANSIToUnicode( pszMessage, szUnlocalizedStr, 512 );
...
bool CHudHintCenter::TranslateKeyCommand( wchar_t *psHintMessage )
{
	...
	for (;;)
	{
		if ( psHintMessage[i] == '{' )
		{
			// YoYo178: SDK 2013 returns key name with spaces on both sides, so this isn't needed
			...
			for (;;)
			{
				...
				if ( psHintMessage[i] == '}' )
				{
					...
					//Msg( "\nConverted Command: %s\n", sConvertedCommand );
					m_pRichText->InsertString(sConvertedCommandUpper);
					// YoYo178: SDK 2013 returns key name with spaces on both sides, so this isn't needed
					//m_pRichText->InsertChar( ' ' );
					i++;
					break;
