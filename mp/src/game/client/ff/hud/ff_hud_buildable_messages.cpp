...
void CHudBuildableMessages::MsgFunc_Dispenser_EnemiesUsing( bf_read &msg )
{
	...
	if( iRead )
	{
		...
		wchar_t *pszTemp = g_pVGuiLocalize->Find( "#FF_DISPENSER_ENEMIESUSING" );
		if( pszTemp )
			wcscpy( m_pText, pszTemp );
		else
			g_pVGuiLocalize->ConvertANSIToUnicode( "Enemies are using your dispenser!", m_pText, sizeof( m_pText ) );
...
void CHudBuildableMessages::MsgFunc_Dispenser_TouchEnemy( bf_read &msg )
{
	...
	if( iRead )
	{
		...
		m_flStartTime = gpGlobals->curtime;
		g_pVGuiLocalize->ConvertANSIToUnicode( szText, m_pText, sizeof( m_pText ) );
		CalculateWidthHeight();
	}
}
...
void CHudBuildableMessages::MsgFunc_Dispenser_Destroyed( bf_read &msg )
{
	...
	if( iRead )
	{
		...
		wchar_t *pszTemp = g_pVGuiLocalize->Find( "#FF_DISPENSER_DESTROYED" );
		if( pszTemp )
			wcscpy( m_pText, pszTemp );
		else
			g_pVGuiLocalize->ConvertANSIToUnicode( "Your dispenser has been destroyed!", m_pText, sizeof( m_pText ) );
...
void CHudBuildableMessages::MsgFunc_SentryGun_Destroyed( bf_read &msg )
{
	...
	if( iRead )
	{
		...
		wchar_t *pszTemp = g_pVGuiLocalize->Find( "#FF_SENTRYGUN_DESTROYED" );
		if( pszTemp )
			wcscpy( m_pText, pszTemp );
		else
