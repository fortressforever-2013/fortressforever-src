...
ConVar cam_idealdist( "cam_idealdist", "128", FCVAR_ARCHIVE | FCVAR_CHEAT );	 // thirdperson distance
...
void CInput::CAM_Think( void )
{
	...
	else
	{
		...
		if( std::fabs( camOffset[ DIST ] - cam_idealdist.GetFloat() ) < 2.0f )
			camOffset[ DIST ] = cam_idealdist.GetFloat();
		else
...
void CInput::CAM_CameraThirdThink( void )
{
	...
	if( std::fabs( vecCamOffset[DIST] - m_pCameraThirdData->m_flDist ) < 2.0f )
	{
