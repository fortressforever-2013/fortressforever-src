...
//	vec[1] *= -1.0f;
	...
	*pYVec -= vecViewDir * DotProduct( vecViewDir, *pYVec );
	...
	size.x *= std::fabs( DotProduct( vec[vecIdx[0]], *pXVec ) );
	size.y *= std::fabs( DotProduct( vec[vecIdx[1]], *pYVec ) );

	// Add the third component into x and y
	size.x += boxSize[vecIdx[2]] * std::fabs( DotProduct( vec[vecIdx[2]], *pXVec ) );
	size.y += boxSize[vecIdx[2]] * std::fabs( DotProduct( vec[vecIdx[2]], *pYVec ) );
