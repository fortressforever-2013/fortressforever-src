...
// ff_bot_engineer_move_to_build.cpp
...
#include "bot/behavior/engineer/ff_bot_engineer_build_teleport_exit.h"
...
ConVar ff_bot_debug_sentry_placement( "ff_bot_debug_sentry_placement", "0", FCVAR_CHEAT );
ConVar ff_bot_max_teleport_exit_travel_to_point( "ff_bot_max_teleport_exit_travel_to_point", "2500", FCVAR_CHEAT, "In an offensive engineer bot's tele exit is farther from the point than this, destroy it" );
ConVar ff_bot_min_teleport_travel( "ff_bot_min_teleport_travel", "3000", FCVAR_CHEAT, "Minimum travel distance between teleporter entrance and exit before engineer bot will build one" );
...
// 			}
	...
	FOR_EACH_VEC( m_sentryAreaVector, it )
	{
		...
		if ( ff_bot_debug_sentry_placement.GetBool() )
		{
		...
		if ( myTeleportExit )
		{
			...
			if ( point )
			{
				...
				if ( pointArea && exitArea )
				{
					...
					if ( travelToPoint > ff_bot_max_teleport_exit_travel_to_point.GetFloat() )
					{
						// too far, destroy it
		...
		else
		{
			...
			if ( myTeleportEntrance && myArea )
			{
				...
				if ( enterArea )
				{
					...
					if ( travelBetween < ff_bot_min_teleport_travel.GetFloat() )
					{
