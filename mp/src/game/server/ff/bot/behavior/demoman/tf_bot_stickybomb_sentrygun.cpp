...
// ff_bot_stickybomb_sentrygun.cpp
...
ConVar ff_bot_sticky_base_range( "ff_bot_sticky_base_range", "800", FCVAR_CHEAT );
ConVar ff_bot_sticky_charge_rate( "ff_bot_sticky_charge_rate", "0.01", FCVAR_CHEAT, "Seconds of charge per unit range beyond base" );
	...
	else if ( stickyLauncher->m_flNextPrimaryAttack < gpGlobals->curtime )
	{
		...
		if ( !m_hasTarget )
		{
			...
			for( int t=0; t<trials; ++t )
			{
				...
				float charge = 0.0f;
				if ( toSentry.IsLengthGreaterThan( ff_bot_sticky_base_range.GetBool() ) )
				{
