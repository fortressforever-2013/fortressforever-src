...
// ff_bot_destroy_enemy_sentry.cpp
...
extern ConVar ff_bot_sticky_base_range;

ConVar ff_bot_debug_destroy_enemy_sentry( "ff_bot_debug_destroy_enemy_sentry", "0", FCVAR_CHEAT );
ConVar ff_bot_max_grenade_launch_at_sentry_range( "ff_bot_max_grenade_launch_at_sentry_range", "1500", FCVAR_CHEAT );
ConVar ff_bot_max_sticky_launch_at_sentry_range( "ff_bot_max_sticky_launch_at_sentry_range", "1500", FCVAR_CHEAT );
...
bool FindGrenadeAim( CFFBot *me, CBaseEntity *target, float *aimYaw, float *aimPitch )
{
	...
	if ( toTarget.IsLengthGreaterThan( ff_bot_max_grenade_launch_at_sentry_range.GetFloat() ) )
	{
		return false;
	}
...
bool FindStickybombAim( CFFBot *me, CBaseEntity *target, float *aimYaw, float *aimPitch, float *aimCharge )
{
	...
	if ( toTarget.IsLengthGreaterThan( ff_bot_max_sticky_launch_at_sentry_range.GetFloat() ) )
	{
		return false;
	}
	...
	*aimCharge = 1.0f;
	...
	{
		float charge = 0.0f;
// 		if ( toTarget.IsLengthGreaterThan( ff_bot_sticky_base_range.GetBool() ) )
// 		{
...
void CFFBotDestroyEnemySentry::ComputeSafeAttackSpot( CFFBot *me )
{
	...
	for( i=0; i<collector.m_area.Count(); ++i )
	{
		...
		if ( ( farthestFromSentry - sentry->GetAbsOrigin() ).IsLengthGreaterThan( SENTRY_MAX_RANGE ) )
		{
			...
			if ( ff_bot_debug_destroy_enemy_sentry.GetBool() )
			{
