...
#define GREN_THROWSPEED 660.0f
	...
	CON_COMMAND( model_temp, "Spawn a temporary model. Usage: model_temp <model> <distance_in_front_of_player>" )
	{
		if( ( args.ArgC() > 2 ) && ( args.ArgC() < 4 ) )
		{
			...
			vecOrigin += ( vecForward * atof( args.Arg(2) ) );
...
void CFFPlayer::Command_TestCommand(const CCommand& args)
{
...
void CFFPlayer::Command_MapGuide( const CCommand& args )
{
	...
	if (args.ArgC() > 1)
		m_hLastMapGuide = m_hNextMapGuide = FindMapGuide(MAKE_STRING(args.Arg(1)));
	else
...
void CFFPlayer::Command_SetChannel(const CCommand& args)
{
	if( args.ArgC() < 2 )
	{
...
void CFFPlayer::KillPlayer( void )
{
	...
	if (IsAlive())
	{
		...
		Event_Dying(CTakeDamageInfo(NULL, NULL, 0, DMG_NEVERGIB));
	}
	else
	{
...
void CFFPlayer::Command_BuildDetpack(const CCommand& args)
{
...
/*			case LUA_EF_CALTROP:
			{
				AddSpeedEffect( SE_CALTROP, flEffectDuration, flSpeed, SEM_ACCUMULATIVE | SEM_HEALABLE, FF_STATUSICON_CALTROPPED, flIconDuration, true );
			}
			break;
...
bool CFFPlayer::LuaIsEffectActive( int iEffect )
{
	switch( iEffect )
	{
		...
		//case LUA_EF_CALTROP: return IsSpeedEffectSet( SE_CALTROP); break;
...
int CFFPlayer::OnTakeDamage(const CTakeDamageInfo &inputInfo)
{
	...
	if ( pVehicle )
	{
		...
		info.ScaleDamage(pVehicle->PassengerDamageModifier(info));
	}
...
void CFFPlayer::Command_Disguise(const CCommand& args)
{
	...
	if(args.ArgC() >= 3)
	{
		const char *szTeam = args.Arg( 1 );
		const char *szClass = args.Arg( 2 );
