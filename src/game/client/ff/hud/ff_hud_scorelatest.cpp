...
void CHudPlayerLatestScore::MsgFunc_SetPlayerLatestFortPoints( bf_read &msg )
{
	...
	if( pszTemp )
		...
		g_pVGuiLocalize->ConvertANSIToUnicode( szString, m_pTextDesc, sizeof( m_pTextDesc ) );

	// convert int-string to unicode
	g_pVGuiLocalize->ConvertANSIToUnicode( szString2, m_pTextScore, sizeof( m_pTextScore ) );
