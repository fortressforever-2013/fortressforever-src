...
// Purpose:
//-----------------------------------------------------------------------------
void CFFTriggerClip::LUA_SetClipFlags( const luabridge::LuaRef& hTable )
{
	SetClipMask( 0 );

	if( hTable.isValid() && hTable.isTable() )
	{
		// Iterate through table
		for( luabridge::Iterator ib( hTable ); !ib.isNil(); ++ib )
		{
			luabridge::LuaRef value = hTable[ib.key()];
