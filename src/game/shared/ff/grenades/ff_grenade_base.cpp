...
/*		float flSpeedSqr = DotProduct(vecVelocity, vecVelocity);
	...
	ConVar target_clamp_min("ffdev_target_clamp_min", "5.0", FCVAR_FF_FFDEV_CLIENT);
	ConVar target_clamp_max("ffdev_target_clamp_max", "60.0", FCVAR_FF_FFDEV_CLIENT); // 30
	ConVar target_size_base("ffdev_target_size_base", "1.0", FCVAR_FF_FFDEV_CLIENT);	// 15
	ConVar target_size_multiplier("ffdev_target_size_multiplier", "6.0", FCVAR_FF_FFDEV_CLIENT); // 15
	ConVar target_time_remaining("ffdev_target_time_remaining", "3.0", FCVAR_FF_FFDEV_CLIENT);

	ConVar target_speed_max("ffdev_target_speed_max", "100", FCVAR_FF_FFDEV_CLIENT);
	ConVar target_speed_min("ffdev_target_speed_min", "20", FCVAR_FF_FFDEV_CLIENT);

	ConVar target_rotation("ffdev_target_rotation", "-118.2", FCVAR_FF_FFDEV_CLIENT); // -100
...
//		float flRotation = gpGlobals->curtime * target_rotation.GetFloat() - anglemod(m_flSpawnTime);
		...
		if (pMaterialBlur)
		{
			...
			pMatRenderContext->Bind(pMaterialBlur);
