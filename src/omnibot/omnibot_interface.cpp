...
// overflow; assigning 1 to bit-field that can only hold values from -1 to 0
// line 90 and 95
...
#pragma warning(disable: 4062) // enumerator 'identifier' in switch of enum 'enumeration' is not handled
	...
	void omnibot_interface::OmnibotCommand(const CCommand &cArgs) 
	{
		if(IsOmnibotLoaded())
		{
			Arguments args;
			...
			{
				Q_strncpy(args.m_Args[args.m_NumArgs++], cArgs.Arg(i), Arguments::MaxArgLength);
			}
...
//---------------------------------------------------------------------------------
PLUGIN_RESULT OmnibotServerPlugin::ClientCommand(edict_t* pEntity, const CCommand& args)
{
...
//---------------------------------------------------------------------------------
void OmnibotServerPlugin::OnQueryCvarValueFinished(QueryCvarCookie_t iCookie, edict_t* pPlayerEntity, EQueryCvarValueStatus eStatus, const char* pCvarName, const char* pCvarValue)
{
	Msg("Cvar query (cookie: %d, status: %d) - name: %s, value: %s\n", iCookie, eStatus, pCvarName, pCvarValue);
}
void OmnibotServerPlugin::OnEdictAllocated(edict_t* edict)
{
}
void OmnibotServerPlugin::OnEdictFreed(const edict_t* edict)
{
}
